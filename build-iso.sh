...
apt-get install -y --no-install-recommends \
  ...
  xterm feh unclutter \
  ca-certificates \
  libgtk-3-0t64 libnss3 libxss1 libgbm1 libdrm2 \
  libatk-bridge2.0-0t64 libxkbcommon0 libcups2t64 \
  surf \
  libasound2t64
# NOTE: chromium-browser was removed on purpose. On Ubuntu 24.04 that
# apt package is only a snap wrapper, and snaps cannot run inside a
# live-boot squashfs — X would start, the kiosk browser would silently
# fail, and boot dead-ended at a text login. The UI now runs in
# Electron (installed below via npm), matching how the StageOS apps
# already run. The libgtk/libnss/libgbm set is Electron's runtime deps.

# ── Xorg: make X actually start under nomodeset ──────────────
# Without this, startx dies with 'no screens found': the default
# 'modesetting' driver needs KMS, which nomodeset disables, so X
# finds no usable device and the kiosk never launches (system falls
# back to the text login). The fbdev driver renders to the kernel
# framebuffer and works under nomodeset on essentially any hardware
# (QEMU, ThinkCentre, Pi). vesa is listed as a secondary fallback.
mkdir -p /etc/X11/xorg.conf.d
cat > /etc/X11/xorg.conf.d/10-fbdev.conf << 'XORGCONF'
Section \"Device\"
    Identifier  \"StageOS Framebuffer\"
    Driver      \"fbdev\"
    Option      \"ShadowFB\" \"true\"
EndSection

Section \"Screen\"
    Identifier  \"StageOS Screen\"
    Device      \"StageOS Framebuffer\"
EndSection
XORGCONF

# Allow X to be started by our non-root 'stageos' user from the
# console (xserver-xorg-legacy defaults to console-only, but make it
# explicit so a startx from the autologin session is permitted).
...
# Hide the mouse cursor when idle (kiosk feel)
unclutter -idle 3 &

# Disable screen blanking / DPMS so the display never sleeps mid-show
xset s off &
xset -dpms &
xset s noblank &

# Start the StageOS output daemon (event relay hub on ws://localhost:9001)
node /opt/stageos/services/output-daemon.js > /tmp/daemon.log 2>&1 &

# Give the daemon a moment to bind its socket before the UI connects
sleep 2

# Launch the launcher UI in kiosk mode, and RELAUNCH it if it ever
# exits — a crashed browser mid-service should recover on its own
# rather than leaving the operator staring at a blank screen.
while true; do
  if command -v electron >/dev/null; then
    electron /opt/stageos/launcher-app --no-sandbox
  else
    # WebKit kiosk fallback (used when the Electron binary could not
    # be bundled at build time). -F = fullscreen.
    surf -F file:///opt/stageos/apps/launcher.html
  fi
  sleep 2
done &
AUTOSTART
...
cp -r os/launcher-app      stageos-build/chroot/opt/stageos/launcher-app
...
# ── Electron runtime (kiosk shell + all StageOS apps) ─────────
# Installed globally so 'electron <appdir>' works from the openbox
# autostart and from the daemon's launchApp(). The postinstall step
# downloads the prebuilt binary from GitHub releases; same cert
# handling and retry as above. Hard-fail if the binary isn't there,
# because without it the OS boots to a black screen instead of a UI.
export ELECTRON_CACHE=/tmp/electron-cache
export npm_config_cafile=/etc/ssl/certs/ca-certificates.crt
export NODE_EXTRA_CA_CERTS=/etc/ssl/certs/ca-certificates.crt
if ! npm install -g electron@31 --unsafe-perm; then
  echo '>>> Electron install attempt 1 failed, retrying...'
  sleep 3
  npm install -g electron@31 --unsafe-perm || true
fi
if [ -x /usr/local/lib/node_modules/electron/dist/electron ]; then
  echo '>>> Electron runtime installed OK'
else
  # Not fatal: the openbox autostart falls back to the surf WebKit
  # kiosk so the OS still boots to the full launcher UI. Electron is
  # still REQUIRED for launching the real StageOS apps — on GitHub
  # Actions (the production build path) this download succeeds; this
  # fallback exists for restricted/proxied build environments.
  echo 'WARNING: Electron binary download failed — ISO will use the'
  echo 'WARNING: surf WebKit kiosk fallback for the launcher UI.'
  npm uninstall -g electron >/dev/null 2>&1 || true
fi
